...
library Roles {
    ...
    // On the escrow:
    //
    // - change borrower, if borrower wallet gets compromised
    // - change payment token, if borrower desires to change the asset used in the offering
    ...
    //
    // - DEFAULT_ADMIN_ROLE = grant/revoke roles (brickken)
    // - ESCROW_WITHDRAW_ROLE = who can withdraw / partially withdraw to borrower (borrower);
    // - ESCROW_NEW_OFFERING_ROLE = starts a new offering (borrower);
    // - ESCROW_OFFERING_FINALIZER_ROLE = finalize an offering (brickken, borrower);
    // - ESCROW_ERC20WHITELIST_ROLE = add/remove ERC20 from whitelist (brickken, borrower);
    // - ESCROW_OFFCHAIN_REPORTER_ROLE = report offchain USD tickets for current offering (borrower)
    //
    // On the token:
    //
    // - change borrower, if borrower wallet gets compromised
    // - change payment token, if borrower desires to change the asset used to repay debt
    ...
    //
    // - DEFAULT_ADMIN_ROLE = grant/revoke roles (brickken)
    // - TOKEN_URL_ROLE = change url (brickken,borrower);
    // - TOKEN_DEBT_REPAY_MANAGER_ROLE = debt repayment manager (borrower);
    // - TOKEN_MINTER_ROLE = mint new tokens (borrower, escrow contract);
    // - TOKEN_MINTER_ADMIN_ROLE = add/remove minters (borrower);
    // - TOKEN_WHITELIST_ADMIN_ROLE = change lenders whitelist (borrower);
    // - TOKEN_WHITELIST_ROLE = whether the user is whitelisted or not (borrower);
    ...
    // - TOKEN_DEBT_REPAY_TOGGLER_ROLE = toggle the debt repay (escrow);
    //
    // On the factory:
    //
    ...
    // - FACTORY_BORROWER_ROLE = whitelisted borrowers (brickken by default);
    ...
    bytes32 public constant FACTORY_BORROWER_ROLE =
        keccak256(abi.encode("FACTORY_BORROWER_ROLE")); // whitelisted borrowers (brickken by default);
    // 0x86f9504d4f12a701ad18b53caa9c998d6c94b96c565c67e086b4b64963391c15
    bytes32 public constant FACTORY_WHITELISTER_ROLE =
        keccak256(abi.encode("FACTORY_WHITELISTER_ROLE")); // can whitelist new borrowers (brickken);

    // 0x7e634896b27e79a960e89ae00f5b87ef365e84b0543277e0eb9d018a47e752f1
    bytes32 public constant ESCROW_WITHDRAW_ROLE =
        keccak256(abi.encode("ESCROW_WITHDRAW_ROLE")); // who can partially withdraw to borrower (borrower);
    // 0x373a3fc4287a1339d038c211cc53ad6aed3b80e2257f4889df84173c57e41680
    bytes32 public constant ESCROW_NEW_OFFERING_ROLE =
        keccak256(abi.encode("ESCROW_NEW_OFFERING_ROLE")); // starts a new offering (borrower);
    // 0x2b797e16dca25f832ef5eecf31aa514d12de6a1d3acdb0e8adae46f707e2d689
    bytes32 public constant ESCROW_ERC20WHITELIST_ROLE =
        keccak256(abi.encode("ESCROW_ERC20WHITELIST_ROLE")); // add/remove ERC20 from whitelist (brickken, borrower);
    // 0xb33eb5fac4455e424b782c8f7380698677b4c84586b4d8f52049804c691485d4
    bytes32 public constant ESCROW_OFFCHAIN_REPORTER_ROLE =
        keccak256(abi.encode("ESCROW_OFFCHAIN_REPORTER_ROLE")); // report offchain tickets (borrower);
    // 0x5bff9b7fe86d1ecb8e388c7b1441e17acc34d11a5ea2f7d940ec3e3ce4886499
    bytes32 public constant ESCROW_OFFERING_FINALIZER_ROLE =
        keccak256(abi.encode("ESCROW_OFFERING_FINALIZER_ROLE")); // finalize an offering (brickken, borrower);

    bytes32 public constant TOKEN_DEBT_REPAY_TOGGLER_ROLE =
        keccak256(abi.encode("TOKEN_DEBT_REPAY_TOGGLER_ROLE")); // can toggle the debt repayment (brickken, borrower);
    // 0x4e2d8cfba5434336d245506de15fea51aa5dbefbf272b26dec4a6d6cf5be64bb

    bytes32 public constant TOKEN_MINTER_ADMIN_ROLE =
        keccak256(abi.encode("TOKEN_MINTER_ADMIN_ROLE")); // add/remove minters (borrower);
    // 0x8d47e3ef6c6edd6d0cd062221e938c7813759ba93d9dabe49168cbc8e44c0758
    bytes32 public constant TOKEN_WHITELIST_ADMIN_ROLE =
        keccak256(abi.encode("TOKEN_WHITELIST_ADMIN_ROLE")); // change lenders whitelist (borrower);
    ...
    bytes32 public constant TOKEN_URL_ROLE =
        keccak256(abi.encode("TOKEN_URL_ROLE")); // change url (brickken,borrower);
    // 0xafece24e20488bd8efc86502ef37bf1d0863acdcafe60cff7122dc1ee40280c2
    bytes32 public constant TOKEN_MINTER_ROLE =
        keccak256(abi.encode("TOKEN_MINTER_ROLE")); // mint new tokens (borrower, escrow contract);
    // 0xe7fd28cbd94ed64bb8cca17950a38aed85f0745ec696947c8e31b86025ae980a
    bytes32 public constant TOKEN_WHITELIST_ROLE =
        keccak256(abi.encode("TOKEN_WHITELIST_ROLE")); // whether the user is whitelisted or not (borrower);
    ...
    bytes32 public constant TOKEN_DEBT_REPAY_MANAGER_ROLE =
        keccak256(abi.encode("TOKEN_DEBT_REPAY_MANAGER_ROLE")); // Debt repayment manager (borrower);
}
