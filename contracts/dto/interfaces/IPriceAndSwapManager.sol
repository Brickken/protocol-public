...
import { IDTOToken } from "./IDTOToken.sol";
...
interface IPriceAndSwapManager {
    ...
    /// @dev auxiliary struct referring to an lender of a specific offering.
    struct UserDetails {
        bool isUserLender; // whether the user is already an lender or not
        uint256 userAmountOfPaymentTokens; // how many paymentToken tokens the user is putting in
    }

    /// @dev auxiliary struct with all the details of the token used by the lender in the offering.
    ...
    struct EscrowDetails {
        ...
        IDTOToken dtoRelatedToken; // the address of the token related to the tokenization
    ...
    /// @param userDetails whether the user is currently an lender and how many payment tokens is using
    /// @param tokenUsedDetails the details of the token being used by the lender to buy a ticket
