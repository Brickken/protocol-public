...
import { DTOCommonErrors, DTOUniSwapErrors } from "./Errors.sol";
...
/// @title PriceAndSwapManager: contract that handles logic for the DTOEscrowUpgradeable contract. Due to contract size limit we had to hand off this into a separate contract.
...
contract PriceAndSwapManager is IPriceAndSwapManager, Initializable {
    ...
    modifier onlyAllowed() {
        if(!allowedCaller[msg.sender]) revert DTOCommonErrors.UserIsNotWhitelisted(msg.sender);
        _;
    }

    // Only the factory contract can call functions with this modifer
    modifier onlyFactory() {
        if(msg.sender != dtoFactory) revert DTOCommonErrors.UserIsNotAdmin(msg.sender);
        _;
    }

    function initialize(address _factory) external initializer {
        if(_factory == address(0)) revert DTOCommonErrors.NotZeroAddress();
    ...
    function getEstimationDTOToken(
        ...
        if (
            (!escrowDetails.dtoRelatedToken.hasRole(Roles.TOKEN_WHITELIST_ROLE, caller) ) ||
        ...
        uint256 dtoRelatedTokenScale = 10 ** escrowDetails.dtoRelatedToken.decimals();
    ...
    function getUSDPriceOfPaymentToken(
        ...
        if(finalPrice < 0) revert DTOCommonErrors.NotNegativeValue(finalPrice);
    ...
    function getPriceInPaymentToken(
        ...
        if(address(pool) == address(0)) revert DTOCommonErrors.NotZeroAddress();
        ...
        ) revert DTOUniSwapErrors.WrongUniswapConfig();
