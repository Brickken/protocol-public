...
import {DTOCommonErrors, DTOEscrowErrors, DTOTokenErrors, DTOUniSwapErrors} from "../helpers/Errors.sol";
...
/// @title DTOEscrowUpgradeable
/// @custom:security-contact tech@brickken.com
contract DTOEscrowUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    IDTOToken public dtoRelatedToken;

    /// @dev Address of the ERC20 token used for the borrower to withdraw the funds or lenders being refunded
    ...
    /// @dev Address of the borrower of new DTO token offerings
    ...
    /// @dev Issuance Index ---> Address of lender ---> Lender struct
    mapping(uint256 issuanceIndex => mapping(address lenderAddress => Lender info))
        public lenders;
    ...
    /// So that borrower value is preserved.
    ...
    /// @dev It manages swaps between the tokens that lenders wants to use (it must be whitelisted in tokenERC20Whitelist)
    /// and the paymentToken so that the lenders exclusively receives paymenToken. It uses Uniswap v3 so it only works
    ...
    /// @param amountInDTO Amount of DTO tokens offered to the user
    event TicketOffered(
        address indexed lender,
    ...
    /// @param amount amount of DTO tokens being reported
    ...
    /// @dev Method for the user to either redeem the DTO tokens or be refunded in paymentTokens
    /// @param index Index of the issuance
    /// @param onBehalfOf Address that will become the lender and receive the DTO tokens or refund the paymentTokens
    function getTokens(
        uint256 index,
        address onBehalfOf
        ...
        if (!dtoRelatedToken.hasRole(Roles.TOKEN_WHITELIST_ROLE, caller))
    ...
    function buyToken(
        ...
        Lender storage lender = lenders[issuanceIndex][onBehalfOf];
        if (lender.redeemed)
            revert DTOEscrowErrors.TokensAlreadyReedemed(onBehalfOf);
        ...
        {
            /// Logic to store the ticket to the lender
            ...
            uint256 newAmountInDTO = actualValue.mulDiv(
                10 ** dtoRelatedToken.decimals(), // dtoRelatedTokenScale
                issuanceCached.priceInUSD,
                MathUpgradeable.Rounding.Down
            );

            uint256 newAmountInPaymentToken = lender.amountInPaymentToken +
                actualAmount;
            lender.amountInPaymentToken = newAmountInPaymentToken;
            lender.amountInDTO = lender.amountInDTO + newAmountInDTO;
    ...
    function getEstimationDTOToken(
        ...
        return
            priceAndSwapManager.getEstimationDTOToken(
                caller,
                ...
                IPriceAndSwapManager.UserDetails(
                    isLender(issuanceIndex, caller),
                    lenders[issuanceIndex][caller].amountInPaymentToken
                ),
    ...
    /// @param issuanceIndexQueried Index of the issuance
    /// @param user Address of the user/lender
    /// @return True if the has reedemed the DTO tokens
    function isRedeemed(
        uint256 issuanceIndexQueried,
        address user
    ) public view returns (bool) {
        return lenders[issuanceIndexQueried][user].redeemed;
    }
    ...
    function isRefunded(
        ...
        return lenders[issuanceIndexQueried][user].refunded;
    }

    /// @dev Method to validate if an user is an lender in the issuance
    /// @param issuanceIndexQueried Index of the issuance
    /// @param user Address of the user/lender
    ...
    function isLender(
        ...
        return
            lenders[issuanceIndexQueried][user].amountInDTO > 0 &&
            // lenders[issuanceIndexQueried][user].amountInPaymentToken > 0 && Removed to allow offchain reported users to redeem tokens
            !lenders[issuanceIndexQueried][user].redeemed &&
            !lenders[issuanceIndexQueried][user].refunded;
    }

    /// @dev Method to calculate amount of USD left that can be invested
    /// @dev It is equal or less to the max ticket allowed per lender in the issuance
    ...
    function __DTOEscrowUpgradeable_init(
        address _dtoToken,
        ...
        if (_newBorrower == address(0) || _treasuryAddress == address(0))
        ...
        dtoRelatedToken = IDTOToken(_dtoToken);
    ...
    /// @dev Method to change the borrower of the contract
    /// @param newBorrower The new borrower
    function _changeBorrower(address newBorrower) internal {
        if (newBorrower == address(0)) revert DTOCommonErrors.NotZeroAddress();
    ...
    function _changeWithdrawalFee(uint24 newFee) internal {
        if (newFee > MAX_FEE_LIMIT)
            revert DTOEscrowErrors.FeeOverLimits(newFee);
    ...
    function _changeTreasuryAddress(address _newTreasuryAddress) internal {
        if (_newTreasuryAddress == address(0))
            revert DTOCommonErrors.NotZeroAddress();
    ...
    function _newOffering(Issuance calldata newIssuance) internal {
        ...
        ) revert DTOEscrowErrors.IssuanceNotFinalized(caller);
        ...
        ) revert DTOEscrowErrors.InitialValueWrong(caller);

        IDTOToken dtoRelatedTokenCached = dtoRelatedToken;

        if (
            dtoRelatedTokenCached.supplyCap() > 0 &&
            newIssuance.issuanceAmount >
            (dtoRelatedTokenCached.supplyCap() -
                dtoRelatedTokenCached.totalSupply())
    ...
    function _finalizeIssuance(address withdrawTo) internal nonReentrant {
        ...
        if (isSuccess(issuanceIndexCached)) {
            ...
            dtoRelatedToken.toggleDebtRepayment(true);
        } else {
            _rollBack();
        }
    }
    ...
    function _checkIssuanceCompleteness(
        ...
        if (index == 0) revert DTOEscrowErrors.IssuanceNotStarted(caller);

        if (!isStarted(index))
            revert DTOEscrowErrors.IssuanceNotStarted(caller);

        if (!isEnded(index) && !isHardcapReached(index))
            revert DTOEscrowErrors.IssuanceNotEnded(
                caller,
                issuances[index].endDate
            );
    }
    ...
    function _checkValidStatus(
        ...
        if (onBehalfOf != caller) {
            if (
                !dtoRelatedToken.hasRole(Roles.TOKEN_WHITELIST_ROLE, onBehalfOf)
            ) revert DTOCommonErrors.UserIsNotWhitelisted(onBehalfOf);
        }

        if (issuanceIndexCached == 0)
            revert DTOEscrowErrors.IssuanceNotStarted(onBehalfOf);

        if (!isStarted(issuanceIndexCached))
            revert DTOEscrowErrors.IssuanceNotStarted(onBehalfOf);

        if (isEnded(issuanceIndexCached))
            revert DTOEscrowErrors.IssuanceEnded(onBehalfOf, endDate);

        if (!tokenERC20Whitelist[tokenUsed].status)
            revert DTOEscrowErrors.TokenIsNotWhitelisted(tokenUsed, onBehalfOf);

        if (raisedAmount == hardCap) revert DTOEscrowErrors.HardCapRaised();
    }
    ...
    /// @param onBehalfOf Address of the lender
    ...
    function _validateInputs(
        ...
        if (
            ...
            revert DTOEscrowErrors.InsufficientAmount(
                onBehalfOf,
                usdValue,
                minTicket
            );
        ...
        if (usdValue > maxTicket)
            revert DTOEscrowErrors.AmountExceeded(
                onBehalfOf,
                usdValue,
                maxTicket
            );

        // If lender already invested previously, check that his total ticket doesn't exceed maxTicket
        if (isLender(issuanceIndex, onBehalfOf)) {
            uint256 amountInDTO = lenders[issuanceIndex][onBehalfOf]
                .amountInDTO;

            uint256 actualAmountInPaymentToken = amountInDTO
                .mulDiv(
                    issuanceCached.priceInUSD,
                    10 ** dtoRelatedToken.decimals(), // dtoRelatedTokenScale,
                    MathUpgradeable.Rounding.Up
    ...
    /// @dev Internal method for the lender to redeem the DTO tokens bought
    /// @param index Index of the issuance
    /// @param onBehalfOf Address that will become the lender and receive the DTO tokens
    function _redeemToken(uint256 index, address onBehalfOf) internal {
        if (isRedeemed(index, onBehalfOf))
            revert DTOEscrowErrors.RedeemedAlready(onBehalfOf, index);

        if (!isLender(index, onBehalfOf))
            revert DTOEscrowErrors.NotLender(onBehalfOf, index);

        if (!isWithdrawn(index) && issuances[index].partialWithdraw == 0)
            revert DTOEscrowErrors.IssuanceNotWithdrawn(index);

        uint256 amountInDTO = lenders[index][onBehalfOf].amountInDTO;

        lenders[index][onBehalfOf].redeemed = true;

        dtoRelatedToken.mint(onBehalfOf, amountInDTO);

        emit Redeemed(onBehalfOf, index, amountInDTO);
    }

    /// @dev Internal method for the lender to be refunded during paymentToken
    /// @param index Index of the issuance
    /// @param onBehalfOf Address that will become the lender and refund the paymentTokens
    function _refundToken(uint256 index, address onBehalfOf) internal {
        if (isRefunded(index, onBehalfOf))
            revert DTOEscrowErrors.RefundedAlready(onBehalfOf, index);

        if (!isLender(index, onBehalfOf))
            revert DTOEscrowErrors.NotLender(onBehalfOf, index);

        if (!isRollback(index))
            revert DTOEscrowErrors.IssuanceNotInRollback(index);

        uint256 amountInPaymentToken = lenders[index][onBehalfOf]
        ...
        lenders[index][onBehalfOf].refunded = true;
    ...
    function _partialWithdraw(
        ...
        if (feeCoverageShortfall > 0) {
            revert DTOEscrowErrors.FeeExceedsBalance(
                feeToPay,
    ...
    function _rollBack() internal {
        ...
        if (isRollback(issuanceIndexCached))
            revert DTOEscrowErrors.IssuanceWasRollbacked(issuanceIndexCached);
    ...
    function _setPaymentToken(
        ...
        ) revert DTOEscrowErrors.IssuanceNotFinalized(borrower);
        ...
        if (_twapInterval > type(uint32).max || _twapInterval == 0)
            revert DTOUniSwapErrors.BadTwapIntervalValue(_twapInterval);
    ...
    /// @param toAssign is the address of the lender that has to be assigned the offchain payment
    ...
    function _offchainReporting(
        ...
        Lender storage lender = lenders[issuanceIndexCached][toAssign];
        if (lender.redeemed)
            revert DTOEscrowErrors.TokensAlreadyReedemed(toAssign);
        ...
        // We dont update lender.amountInPaymentToken because it didn't deposit any token at all. Instead is an offchain record.
        ...
        emit OffchainReport(newAmountInDTO, toAssign, proofOfPayment);
    }
    ...
    {
        uint256 issuanceIndexCached = issuanceIndex;
        if (!isActive(issuanceIndexCached))
            return (
                hasEnoughBalance,
                feeToPay,
                amountToReceive,
    ...
    function _patchOffering(
        ...
        if (newStart != issuanceCached.startDate) {
            if (newStart < block.timestamp)
                revert DTOEscrowErrors.PatchValueWrong(
