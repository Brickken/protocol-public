...
interface IDTOFactoryUpgradeable {
    ...
    /// @param name Name of the DTOToken
    /// @param symbol Symbol of the DTOToken
    /// @param supplyCap Cap on the supply of the DTOToken, 0 if unlimited supply (unlimited up to type(uint224).max because of checkpoints feature)
    /// @param paymentToken Token used to denominate borrower's withdraw on succesfull tokenization
    ...
    /// @param preMints Amounts of the DTOToken to be minted to each initial holder
    /// @param initialHolders Wallets of the initial holders of the DTOToken
    ...
    /// @dev Event emitted when a new DTO is created
    /// @param id ID of the DTO
    /// @param token smart contract address of the DTOToken
    /// @param escrow smart contract address of the DTOEscrow
    event NewDebtTokenization(
        uint256 indexed id,
    ...
    /// @param borrower wallet address of the borrower
    /// @param currency token used to pay the fee, usually BKN
    /// @param amount Amount of fees charged
    event ChargeFee(address indexed borrower, string currency, uint256 amount);
    ...
    /// @param newPriceInBKN New price in BKN
    /// @param newPriceInUSD New price in USD
    function changeFee(uint256 newPriceInBKN, uint256 newPriceInUSD) external;
    ...
    function changeBKNPriceTWAPInterval(uint256 newTWAP) external;
    ...
    /// @param credits number of credits for new tokenization of above users to be set
