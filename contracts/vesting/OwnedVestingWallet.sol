...
contract VestingWallet is Ownable, ReentrancyGuard {
    ...
    */
     ...
     */
    function delegate(address token, address delegatee) external nonReentrant() onlyBeneficiary() {
        ERC20Votes(token).delegate(delegatee);
    }

    function setBeneficiary(address recipient_) external nonReentrant() onlyOwner() {
